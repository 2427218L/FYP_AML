...
def make_step(net, xy, step_size=1.5, end='fc8', clip=True, unit=None, denoise_weight=0.1, margin=0, w=224, h=224):
    ...
    if (np.abs(g).mean() == 0):
        print('absolute mean is too small.')
    ...
    # Run a denoising process on the resultant image
...
def main():
    ...
    # which class to visualize (target class)
    unit = int(sys.argv[1])
    ...
    start_image = np.float32(scipy.misc.imread('abigail.jpg'))
